...
from model.BasicLayer import FeaturesLinear, FeaturesEmbedding, MultiLayerPerceptron, BasicCTRLLM, FeaturesLinear


class WDL(BasicCTRLLM):
    ...
    def forward(self, user_item_ids):
        ...
        y_pred = self.output_layer(x_fi) + self.LR(user_item_ids) # B, 1
        
        y_pred = self.output_activation(y_pred).squeeze(1)
        return_dict = {"y_pred": y_pred}
        return return_dict

class WDL_PLUG(BasicCTRLLM):
    ...
    def forward(self, user_item_ids, return_features=False):
        ...
        y_pred = self.compute_logits(x_fi) + self.LR(user_item_ids)
